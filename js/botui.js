  ...
  var sure = function () {
    ...
    secondpart = function () {
      botui.message.add({
        ...
        botui.message.add({
          delay: 1500,
          content: "从小立志成为一名光荣的园丁，浇灌祖国的花朵。"
        }).then(function () {
          botui.message.add({
            delay: 1200,
            content: "但是却步入了码农的苦逼到道路，但后来也逐渐喜欢上了coding…"
          }).then(function () {
            botui.message.add({
              delay: 1500,
              content: "主攻前端(HTML/JS/CSS/Vue)，略懂后端(JAVA)，偶尔也玩玩数据分析和可视化(PYTHON)"
            }).then(function () {
              botui.message.add({
                delay: 1500,
                content: "有时也玩玩爬虫，感觉特别有意思。"
              }).then(function () {
                botui.message.add({
                  delay: 1800,
                  content: "喜欢打羽毛球，曾梦想成为一名职业羽毛球选手"
                }).then(function () {
                  botui.action.button({
                    delay: 1100,
                    action: [{
    ...
    thirdpart = function () {
      botui.message.add({
        delay: 1E3,
        content: "曙光星云来源于中国自主研发的第一台实测性能超千万亿次的超级计算机，在那一年我终于有了我第一台笔记本电脑，因为印象比较深刻，所以对曙光星云这四个字有着非同一般的执着。"
      }).then(function () {
        botui.action.button({
          delay: 1500,
          action: [{
    ...
    fourthpart = function () {
      botui.message.add({
        delay: 1E3,
        content: "因为Sunny有着阳光、快乐、开朗的寓意"
      }).then(function () {
        botui.message.add({
          delay: 1100,
          content: "而且我现实中我真的是一个Sunny的蓝孩子！"
        }).then(function () {
          botui.action.button({
            delay: 1500,
            action: [{
              text: "博客为什么没有使用域名解析？(ง •_•)ง",
              value: "why-domain？"
            }]
          }).then(function (a) {
            fifthpart()
    ...
    fifthpart = function () {
      botui.message.add({
        delay: 1E3,
        content: "emmmm，之前有过一个域名也备案成功了，但是过期未续费最后被别人注册了.......后悔死了。"
      }).then(function () {
        botui.message.add({
          delay: 1600,
          content: "哎不提伤心事了，仔细看看我的博客吧？ ^_^"
        })
      })
    }
}